      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['system:channel:list:unchecked']"
          >详情</el-button>
          <el-button
            size="mini"
            type="text"
            ...
            v-hasPermi="['system:channel:check']"
          >通过</el-button>
          <el-button
            size="mini"
            type="text"
...
<script>
import { listFirstColums, getChannel, delChannel, addChannel, checkChannel } from "@/api/system/channel";
...
export default {
  ...
  methods: {
    ...
    handlePass(row) {
      row.remark = 1;
      console.log(row)
      checkChannel(row).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.newsId != null) {
            checkChannel(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
