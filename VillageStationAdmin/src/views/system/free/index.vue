...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.askFreeId)
    ...
    handleUpdate(row) {
      this.reset();
      const askFreeId = row.askFreeId || this.ids
      getFree(askFreeId).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.askFreeId != null) {
            updateFree(this.form).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const askFreeIds = row.askFreeId || this.ids;
      this.$modal.confirm('是否确认删除随时问编号为"' + askFreeIds + '"的数据项？').then(function() {
        return delFree(askFreeIds);
      }).then(() => {
        this.getList();
