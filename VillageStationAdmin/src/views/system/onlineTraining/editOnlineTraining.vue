<template>
  <div>
    ...
    <div class="upload_row">
      ...
      <el-upload ref="upload" :limit="1" accept=".mp4" :action="uploadVideo.url" :headers="uploadVideo.headers"
        list-type="picture-card" 
        :before-upload="handleBeforeVideoUpload"
        :on-progress="handleUploadProgress" :on-success="handleVideoSuccess"
    ...
    <!-- 编辑器  -->
    <el-upload :action="uploadUrl" :before-upload="handleImageBeforeUpload" :on-success="handleUploadSuccess"
...
import { updateTraining } from "@/api/onlineTraining/index";
...
export default {
  ...
  data() {
    ...
    this.form = JSON.parse(localStorage.getItem("editOnlineTraining"))
  ...
  methods: {
    ...
    submit() {
      if (this.form.title == undefined || this.form.title == "" || this.form.title == null) {
        this.$message.error("请输入线上培训标题");
        return
      } else if (this.form.sort == undefined || this.form.sort == "" || this.form.sort == null) {
        this.$message.error("请选择线上培训标签");
        return
      }
      if (this.uploadVideo.isUploading == true) {
        this.$message.error("视频上传中，请等待视频上传完成")
        return
      }
      ...
      updateTraining(this.form).then(response => {
        ...
        this.open = false;
        this.$router.push('/information/submit/training/online');
      });
    },
    //富文本编辑器
    init() {
    ...
    handleImageBeforeUpload(file) {
      ...
      if (this.imageFileSize) {
        ...
        if (!isLt) {
          this.$message.error(`上传文件大小不能超过 ${this.imageFileSize} MB!`);
          return false;
        }
      }
      return true;
