  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getTraining(id).then(response => {
        localStorage.setItem("editOnlineTraining",JSON.stringify(response.data))
        this.$router.push('/edit/editOnlineTraining')
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/online_training/export', {
        ...this.queryParams
      }, `online_training_${new Date().getTime()}.xlsx`)
    },
    showType2(str){
