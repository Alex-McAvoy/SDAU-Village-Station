<template>
  <div>
    ...
    <div class="inputItem" style="display:grid;grid-template-columns: auto 3fr 1fr  ;margin:10px 50px 5px 5px ">
      ...
      <el-input  style="grid-column:2/3;margin-right: 100px;" v-model="form.title" placeholder="请输入内容" label="标题"></el-input>
      <el-button type="primary" style="grid-column: 3/4;border:0px;border-radius: 3px;margin-left: 30px;" icon="el-icon-upload2" @click="submitForm">修改</el-button>
    </div>
    <!-- 类别 -->
    <div class="inputItem" style="display:flex;margin:5px 50px 5px 5px; ">
      <el-button style="background: #fff;border:0px;width:100px"><span style="color:red">* </span>信息类型</el-button>
      <el-select v-model="form.category" placeholder="请选择驿站信息类型">
    ...
    <!-- 图片 -->
    <div class="inputItem" style="display:flex;margin:5px 50px 5px 5px; ">
      <el-button style="background: #fff;border:0px;width:100px">封面</el-button>
      <el-upload ref="upload" :limit="1" accept=".png,.jpg" :action="uploadImg.url" 
        list-type="picture-card" :on-progress="handleUploadProgress" :on-success="handleImgSuccess" :headers="uploadImg.headers"
        :on-remove="handleRemove" :file-list="uploadImg.fileList">
...
import {  addColumns, updateColumns } from "@/api/stationInformation/index";
...
export default {
  ...
  data() {
    ...
    this.form = JSON.parse(localStorage.getItem("editStationInformation"))
  ...
  methods: {
    ...
    submitForm(){
      ...
      updateColumns(this.form).then(response => { 
        ...
        this.open = false;
        this.$router.push('/station/submit/information');
      });
    },
    init() {
