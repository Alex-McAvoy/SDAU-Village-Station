<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="columnsList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="驿站信息类型" align="center" prop="category" :formatter="showType2"/>
...
<script>
import { getStationInformationByStationId, getColumns, delColumns, addColumns, updateColumns } from "@/api/stationInformation/index";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getStationInformationByStationId(stationId, this.queryParams).then(response => {
    ...
    /** 新增按钮操作 */
    handleAdd() {
      this.$router.push('/addStationInformation')
    },
    /** 修改按钮操作 */
    handleUpdate(row) { 
      const id = row.id || this.ids
      getColumns(id).then(response => {  
        localStorage.setItem("editStationInformation",JSON.stringify(response.data))
        this.$router.push('/edit/editStationInformation')
      });
    },
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/station_information/export', {
        ...this.queryParams
