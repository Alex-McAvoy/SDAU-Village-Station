<template>
  <div>
    ...
    <div class="inputItem" style="display:grid;grid-template-columns: auto 3fr 1fr  ;margin:10px 50px 5px 5px ">
      <!-- <el-button style="background: #fff;border:0px;width: 100px;">标题</el-button>
      <el-input  style="grid-column:2/3;margin-right: 100px;" v-model="form.title" placeholder="请输入内容" label="标题"></el-input> -->
      <el-button type="primary" style="grid-column: 3/4;border:0px;border-radius: 3px;margin-left: 30px;" icon="el-icon-upload2" @click="submit">发布</el-button>
    </div>
     <!-- 标签 -->
     <div class="inputItem" style="display:flex;margin:5px 50px 5px 5px; ">
      ...
      <el-select v-model="form.category" multiple placeholder="请选择标签" required>
...
            <el-option label="农机" value="11"></el-option>
    ...
    <!-- 类别 -->
    <!-- <div class="inputItem" style="display:flex;margin:5px 50px 5px 5px; ">
      <el-button style="background: #fff;border:0px;width:100px">驿站信息类型</el-button>
...
import { addColumns } from "@/api/stationIntroduction/index";
...
export default {
  ...
  methods: {
    ...
    submit() {
      if (this.form.category == undefined || this.form.category.length == 0) {
        this.$message.error("请选择信息标签");
        return
      }
      ...
      this.form.content = this.form.content.replace(/>/g, '&gt;')
      this.form.sort = this.$store.state.user.deptId
      
      addColumns(this.form).then(response => {
        this.$modal.msgSuccess("发布成功");
        this.open = false;
        this.$router.push('/station/submit/introduction');
      });
    }
  },
};
