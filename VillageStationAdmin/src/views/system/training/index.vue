  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="1400px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="审核标志" prop="audit" :disabled="formDisabled">
          <el-input v-model="form.audit" :disabled="formDisabled" />
        </el-form-item>
...
<script>
import { listTraining, getTraining, delTraining, addTraining, updateTraining } from "@/api/offlineTraining/index";
...
export default {
  ...
  methods: {
    ...
    /** 新增按钮操作 */
    handleAdd() {
      this.$router.push('/addOfflineTraining')
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      const id = row.id || this.ids
      getTraining(id).then(response => {
        localStorage.setItem("editOfflineTraining",JSON.stringify(response.data))
        this.$router.push('/edit/editOfflineTraining')
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除线下培训编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/offline_training/export', {
        ...this.queryParams
      }, `offline_training_${new Date().getTime()}.xlsx`)
    },
