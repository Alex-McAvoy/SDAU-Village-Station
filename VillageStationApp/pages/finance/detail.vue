	...
	<view>
		...
		<view class="tab_nav" style="border-radius: 10px; background-color: white;  margin:8px 15px;">
			...
			<view class="news">
				...
				<view class="new_text" style="text-indent: 2em; line-height: 1.8; margin: 20px; margin-top: -10px;" ><u-parse :content="item.content"></u-parse></view>
			</view>
...
<script>
	...
	export default {
		...
		created() {
			...
			Promise.all([
				getFinanceDetail(this.detailId),
				getLikes(this.userInfo.userId, "finance", this.detailId),
				getCollect(this.userInfo.userId, "finance", this.detailId),
			]).then(response => {
		...
		methods: {
			clickLikes() {
				if (this.likesStatus == 0) { // 点赞
					...
					Promise.all([
						subFinanceLikes(temp),
						delLikes(this.userInfo.userId,"finance",this.detailId)
					]).then(response => {
			...
			clickCollect() {
				if (this.collectStatus == 0) { // 收藏
					...
					Promise.all([
						subFinanceCollect(temp),
						delCollect(this.userInfo.userId,"finance",this.detailId)
					]).then(response => {
