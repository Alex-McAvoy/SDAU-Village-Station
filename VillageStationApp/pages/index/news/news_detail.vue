		...
		methods: {
			...
			clickCollect() {
				if (this.collectClickCount % 2 == 0) { // 收藏
					...
					addNewsCollect(this.item).then(response => {}) //增加收藏数
					this.addCollectItem(); //新增收藏条目 
				} else { // 取消收藏
			...
			addLikes() { //新增喜欢
				...
				data.tableName = "news_detail"; //需要后端对应，找到真实的
