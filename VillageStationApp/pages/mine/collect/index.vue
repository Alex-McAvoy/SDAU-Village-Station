...
<script>
	...
	export default {
		...
		created() {
			...
			this.loading = false
			getCollectList(this.userInfo.userId).then(response => {
				...
				if (this.ids != undefined) {
					...
					if (this.ids.station_information == undefined) {
						this.ids.station_information = 0
					}
					...
					Promise.all([
						...
						getStationInformationByIds(this.ids.station_information),
					]).then(response => {
						...
						for (var item of response[4]) {
							item.url = '/pages/station/information_detail'
							item.type = "驿站信息"
							item.index = 4
							this.list.push(item)
						}
		...
		methods: {
			skip(item) {
				...
				if (item.index == 4) {
					updateStationInformationReading(item).then(response => {})
					uni.navigateTo({
						url: item.url
					})
				}
