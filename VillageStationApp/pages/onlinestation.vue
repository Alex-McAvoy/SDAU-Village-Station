...
<!-- 		<div class="nav_bar" style="background-color: white;">
		...
		<view style="background-color: white;margin: 15px; border-radius: 10px;">
			...
			<view style=" font-size: 18px; text-indent: 2em; line-height: 1.8; margin: 20px;">
				建设乡村驿站首先要尊重和保护好乡村生活，有效结合乡村所富有的自然资本和文化资本实现生产空间的重构和环境可持续发展。
				乡村驿站可结合美丽乡村和田园综合体等政策，与当地企业、政府、社会团体等合作，有效集约建设各类生活设施。
				乡村驿站也可以作为一个促进城乡广泛协作的平台，成为破解城乡分割结构的先锋。点击下方地图加入相关驿站。
			</view>
			<view class="box">
			...
			<div v-for="oneCity in city" :key="oneCity.deptId">
				<u-collapse @change="change(oneCity)" style="background-color: white; margin: 15px;">
					<u-collapse-item :title="oneCity.deptName">
						<view v-for="oneStation in station" :key="oneStation.deptId" >
							<u-row
								style=" border-bottom: 1px solid gainsboro; line-height: normal;">
								<u-col :span="6"
									style="margin-left: 5px;font-size: 15px;text-align: center; ">{{oneStation.deptName}}
								</u-col>
								<u-col :span="3" style="flex-direction: column;">
									<u-col style="margin-bottom: 10px;"><button
											style="font-size: 11px; width: 80px;height: 30px; background-color: #00ae67;color: white;">查看详情</button></u-col>
									<u-col style="margin-bottom: 5px;"><button
