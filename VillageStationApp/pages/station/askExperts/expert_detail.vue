			...
			<view style="height: 100%; background-color: white; margin: 15px;  margin-top: 15px; border-radius: 5px;"
				...
				<!-- <view>
					<u-row justify="flex-end" gutter="15">
						<u-col span="8">a</u-col>
						<u-col span="4" style="color:#D3D3D3;font-size: 10px;">
							{{onequestions.createTime}}</u-col>
					</u-row>
				</view> -->
			</view>
		</div>
...
<script>
	...
	export default {
		...
		created() {
			...
			this.getQuestionList(this.expertId)
		},
		onLoad(options) {
		...
		methods: {
			...
			//获取问题列表
			getQuestionList(expertId) {
				this.loading = true;
				listQuestion(expertId, 1).then(response => {
