<template>
	<view>
		...
		<view v-if="question.length!=0">
			...
			<view>
				<u-modal :show="show" :title="title" @cancel="cancel" @confirm="confirm" :showCancelButton='true'>
					<div style="width: 90%;">
						<u--input placeholder="请输入标题1" v-model="newQuestion.title"></u--input>
						<u--input placeholder="请输入内容" style="margin-top: 20px;"
							v-model="newQuestion.content"></u--input>
						<view style="margin-top: 15px;">
							<u-upload :fileList="fileList1" @afterRead="afterRead" @beforeRead="beforeRead" @delete="deletePic" name="1" 
								:maxCount="1"></u-upload>
						</view>
					</div>
				</u-modal>
		...
		<view v-else>
			...
			<view>
				<u-modal :show="show" :title="title" @cancel="cancel" @confirm="confirm" :showCancelButton='true'>
					<div style="width: 90%;">
						...
						<u--input placeholder="请输入内容2" style="margin-top: 20px;"
							v-model="newQuestion.content"></u--input>
						<view style="margin-top: 15px;">
			...
			<!-- 弹框开关 -->
			<view @click="open">
				<u-button v-if="role === 'user' || role === 'admin' " class="custom-style" color="#00ae67 " type="primary" shape="circle"
					style="  width: 60px; height: 60px; position: fixed;bottom: 90px;right: 30px; font-size: 50px;">＋
				</u-button>
			</view>
		</view>
