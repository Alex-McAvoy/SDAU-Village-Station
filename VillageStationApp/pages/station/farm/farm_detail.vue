	...
	<view style="border-radius: 10px; background-color: white; margin:8px 15px; height: 100vh;" :model="farms">
		...
		<view>
			<u-row style="height: 25px; margin-left: 100px;">
				<u-col span="15">
					<u--text color="#cbcbcb" :text="farms.createTime">管理员</u--text>
				</u-col>
			</u-row>
		</view>
		<view class="news">
			<view class="new_img">
				<image :src="farms.remark" alt="" style="width:100%;height: 200px;overflow: hidden">
			</view>
		...
		methods: {
			...
			clickLikes() {
				if (this.likesClickCount % 2 == 0) { // 点赞 
					...
					addFarmLikes(this.farms).then(response => {})
					this.addLikes();
				} else { // 取消点赞
					this.thumbup = '/static/images/tabbar/unlikes.png'
					this.likes = (parseInt(this.likes) - 1).toString()
					subFarmLikes(this.farms).then(response => {})
					this.delLikes();
			...
			clickCollect() {
				if (this.collectClickCount % 2 == 0) { // 收藏
					...
					addFarmCollect(this.farms).then(response => {})
					this.addCollectItem();
				} else { // 取消收藏
					this.star = '/static/images/tabbar/uncollect.png'
					this.collect = (parseInt(this.collect) - 1).toString()
					subFarmCollect(this.farms).then(response => {})
					this.delCollectItem();
				}
			...
			addLikes() { //新增喜欢
				...
				data.tableName = "purchaseFarm"; //需要后端对应，找到真实的
				data.articleId = this.farms.newsId;
				addLikes(data).then(response => {
				});
			},
			delLikes() { //删除喜欢
				var userid = this.userInfo.userId;
				var tablename = "purchaseFarm";
				var articleid = this.farms.newsId;
