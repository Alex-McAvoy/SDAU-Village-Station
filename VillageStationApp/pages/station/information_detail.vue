...
<script>
	import {
		getCheckStationInformationDetails,
		addStationInformationLikes,
		subStationInformationLikes,
		addStationInformationCollect,
		subStationInformationCollect,
	...
	export default {
		...
		created() {
			...
			Promise.all([
				getCheckStationInformationDetails(this.detailId),
				getLikes(this.userInfo.userId, "station_information", this.detailId),
				getCollect(this.userInfo.userId, "station_information", this.detailId),
			]).then(response => {
		...
		methods: {
			clickLikes() {
				if (this.likeStatus == 0) { // 点赞
					...
					Promise.all([
						addStationInformationLikes(temp),
						addLikes({
					...
					temp.content = temp.content.replace(/>/g, "&gt;");
					Promise.all([
						subStationInformationLikes(temp),
						delLikes(this.userInfo.userId,"station_information",this.detailId)
					]).then(response => {
			...
			clickCollect() {
				if (this.collectStatus == 0) { // 收藏
					...
					temp.content = temp.content.replace(/>/g, "&gt;");
					Promise.all([
						addStationInformationCollect(temp),
						addCollect({
							"userId":this.userInfo.userId,
							"tableName": "station_information",
							"articleId": this.detailId
					...
					temp.content = temp.content.replace(/>/g, "&gt;");
					Promise.all([
						subStationInformationCollect(temp),
						delCollect(this.userInfo.userId,"station_information",this.detailId)
					]).then(response => {
