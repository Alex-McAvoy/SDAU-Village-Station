<template>
	<view class="sub_main" style="border-radius: 10px; background-color: white;  margin:8px 15px; height: 100vh;">
		...
		<view class="news">
			<view class="new_img">
				<image :src="imgUrl" alt="" style="width:100%;height: 200px;overflow: hidden">
			</view>
...
<script>
	export default {
		...
		methods: {
			getList() {
				this.loading = true;
				var species_detail = getApp().globalData.item;
				this.title = species_detail.title;
				this.content = species_detail.content;
				this.imgUrl = species_detail.remark;
				this.createTime = species_detail.createTime;
				this.loading = false;
			}
		}
	}
