<template>
	<view style="">
		<!-- 弹出框 -->
		<view>
			<u-modal :show="show" :title="title" :content='content' @confirm="confirm" @cancel="cancel" :showCancelButton="true"></u-modal>
		</view>
		<view class="contain">
			<view>
			...
			<!-- 顶部 搜索栏 -->
			<view class="nav_bar">
				<u-row gutter="20" class="index_head" style="padding-left: 15px;">
					...
					<u-col @click="GoBase" class="flex_row" span="3" style="margin-left: 15px;">
						<u-icon name="home-fill" color="white" size="15" labelPos="bottom" labelColor="white"></u-icon>
						<view style="font-size: 13px;margin-top: -15px;">切换</view>
					</u-col>
				</u-row>
			</view>
			...
			<view class="u-page" v-for="expert in Array.from(experts).slice(0,2)">
				<view class="u-demo-block">
					<view class="album">
						...
						<view class="album__content" @click="skip1(expert)">
							...
							<view class="u-demo-block">
								<view class="u-demo-block__content">
									<view class="album">
										...
										<view class="album__content" @click="skip2(item)">
											<view>
												<u-text style="padding-right: 8px;" margin="20px 0px 8px 0px"
													:text="item.title" bold size="17"></u-text>
											</view>
		...
		<view style="border-radius: 10px; background-color: white;  margin:15px">
			...
			<view class="u-page" v-for="item in Array.from(techDetails).slice(0,2)" @click="goTechDetail(item)">
				<view class="u-demo-block">
					<view class="album">
						<view class="album__avatar">
							<image margin="30px 0px 8px 0px" :src="item.remark"
		...
		<view style="border-radius: 10px; background-color: white;  margin:15px">
			...
			<view class="u-page" v-for="item in Array.from(farmList).slice(0,2)" @click="skip3(item)">
				<view class="u-demo-block">
					<view class="album">
						<view class="album__avatar">
							<image margin="30px 0px 8px 0px" :src="item.remark"
		...
		<view style="border-radius: 10px; background-color: white;  margin:15px">
			...
			<u-tabs :list="channelBarList" @click="getChannelList" lineColor="#2ed573" lineWidth="60"
				:current="0"></u-tabs>
		...
		<view style="border-radius: 10px; background-color: white;  margin:15px">
			...
			<u-tabs :list="productBarList" @click="getProductList" lineColor="#2ed573" lineWidth="60"
				:current="0"></u-tabs>
	...
	export default {
			...
			return {
				...
				barList: [{ //导航栏
					...
					},
					{
						...
						title: '寻基地',
						route: '/pages/station/onlinebase/onlinebase'
					},
				],
				Info: { // 驿站信息
					title: '',
		...
		created() {
			...
			if (this.userInfo.deptId != ''){
				...
				Promise.all([
					getStationListByColumn(1, this.userInfo.deptId),
					getFreeAskListByColumn(1, this.userInfo.deptId),
					getTechListByColumns(1, 0, this.userInfo.deptId),
					getFarmListByColumn(1, 0, this.userInfo.deptId),
					getChannelListByColumns(1, 0, this.userInfo.deptId),
					getProductsBySort(1, 0, this.userInfo.deptId),
					getSpeciesListByColumn(1, this.userInfo.deptId)
				]).then(response => {
					...
					if (response[1].data.length == 0) {
						...
						this.askQuestions = response[1].data;
					}
					if (response[2].data.length == 0) {
						...
						this.techDetails = response[2].data;
					}
					...
					if (response[4].data.length == 0) {
						...
						this.channelList = response[4].data;
					}
					if (response[5].data.length == 0) {
						this.productsList = [{
							title: '',
							content: '该驿站暂无数据',
							remark: ''
						...
						this.productsList = response[5].data;
					}
					if (response[6].data.length == 0) {
						...
						this.speciesList = response[6].data;
					}
		...
		methods: {
			...
			onTabItemTap() {
				if (!this.userInfo.deptId) {
					// 如果 this.userInfo.deptId 为空，则显示弹框
					this.show = true
				} else {
					uni.switchTab({
						url: '/pages/station/station/station',
					});
				}
			},
			//跳转驿站信息列表页
			...
			getTechList(item) {
				Promise.all([
					getTechListByColumns(1, item.index, this.userInfo.deptId)
				]).then(response => {
					if (response[0].data.length == 0) {
						this.techDetails = [{
							title: '',
						...
						this.techDetails = response[0].data;
					}
				})
			},
			//跳转学农技
			...
			getFarmList(item) {
				Promise.all([
					getFarmListByColumn(1, item.index, this.userInfo.deptId)
				]).then(response => {
					if (response[0].data.length == 0) {
						this.farmList = [{
							title: '',
			...
			getChannelList(item) {
				Promise.all([
					getChannelListByColumns(1, item.index, this.userInfo.deptId)
				]).then(response => {
					if (response[0].data.length == 0) {
						this.channelList = [{
							title: '',
						...
						this.channelList = response[0].data;
					}
				})
			},
			//跳转到找渠道
			...
			getProductList(item) {
				Promise.all([
					getProductsBySort(1, item.index, this.userInfo.deptId)
				]).then(response => {
					if (response[0].data.length == 0) {
						this.productsList = [{
							title: '',
						...
						this.productsList = response[0].data;
					}
				})
			},
			//跳转到推优品
			...
			goOnlinebase() {
				uni.navigateTo({
					url: "/pages/station/onlinebase/onlinebase"
				})
			},
			// 控制显示文本长度
			fixedSize(content) {
