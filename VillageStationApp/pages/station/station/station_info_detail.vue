	...
	<view class="sub_main" style="border-radius: 10px; background-color: white;  margin:8px 15px; height: 100vh;">
		...
		<view>
			<u-row style="height: 25px; margin-left: 100px;">
				<u-col span="15">
					<u--text color="#cbcbcb" :text="item.createTime">管理员|</u--text>
				</u-col>
			</u-row>
		</view>
		<view class="news">
		...
		methods: {
			...
			clickLikes() {
				if (this.likesClickCount % 2 == 0) { // 点赞
					...
					addInfoLikes(this.item).then(response => {})
					this.addLikes();
				} else { // 取消点赞
					this.thumbup = '/static/images/tabbar/unlikes.png'
					this.likes = (parseInt(this.likes) - 1).toString()
					subInfoLikes(this.item).then(response => {})
					this.delLikes();
				}
				this.likesClickCount += 1
			},
			clickCollect() {
				if (this.collectClickCount % 2 == 0) { // 收藏
					...
					addInfoCollect(this.item).then(response => {})
					this.addCollectItem()
				} else { // 取消收藏
					this.star = '/static/images/tabbar/uncollect.png'
					this.collect = (parseInt(this.collect) - 1).toString()
					subInfoCollect(this.item).then(response => {})
					this.delCollectItem()
				}
				this.collectClickCount += 1
			},
			addLikes() { //新增喜欢
				...
				data.tableName = "station_info_detail"; //需要后端对应，找到真实的
			...
			delLikes() { //删除喜欢 
				...
				var tablename = "station_info_detail";
