	...
	<view class="body">
		<view class="main_context first_main_context" style="margin-top: 15px;">
			<view v-if="offlineList.length!=0">
				<view class="news" style="height:120px;" v-for="item in offlineList" @click="goOfflineDetail(item)">
					<view class="new_img">
...
<script>
	...
	export default {
		...
		methods: {
			...
			getOfflineList() {
				this.loading = true;
				getOfflineTrainingListTag(this.deptTag,this.queryParam).then(response => {
