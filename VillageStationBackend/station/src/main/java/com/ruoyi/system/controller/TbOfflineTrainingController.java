...
public class TbOfflineTrainingController extends BaseController {
    ...
    //通过id列表来查询
    @GetMapping("/get_offlinetraining_idlist/{idlist}")
    public List<TbOfflineTraining> getNewsByIdList(@PathVariable("idlist")  Long[] idList)
    {
        List<TbOfflineTraining> list = tbOfflineTrainingService.selectTbOfflineTrainingByIdList(idList);
        return  list;
    }
}
